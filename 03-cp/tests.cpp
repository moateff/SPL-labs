...
protected:
    ...
    std::pair<std::string, int> run_cp_command(int argc, char *argv[]) {
        int pipefd[2];
        pid_t pid;
        char buffer[1024];
        std::string output;
        ...
        if (pid == 0) { // Child process
            ...
            // Run the student's cp_main function
            exit(cp_main(argc, argv));
        } else { // Parent process
...
TEST_F(CpTest, CopyFile) {
    ...
    const char *argv[] = {"cp", source, destination, NULL};
    int argc = 3;

    auto result_status = run_cp_command(argc, const_cast<char**>(argv));
    std::string result = result_status.first;
    int status = result_status.second;

    ASSERT_EQ(status, 0) << "cp program should return 0 on success.";
...
TEST_F(CpTest, CopyLargeFile) {
    const char *source = "large_source.txt";
    const char *destination = "large_destination.txt";
    std::string content(10000, 'a'); // Create a large file with 10,000 'a' characters
    ...
    std::string dest_content = read_file(destination);
    ASSERT_STREQ(content.c_str(), dest_content.c_str()) << "The content of the destination file should match the source file.";
...
TEST_F(CpTest, CopyToExistingFile) {
    const char *source = "source.txt";
    const char *destination = "destination.txt";
    const char *content = "This is a test file.";
...
TEST_F(CpTest, SourceFileDoesNotExist) {
    ...
    ASSERT_NE(status, 0) << "cp program should return a non-zero exit status if the source file does not exist.";
...
TEST_F(CpTest, InvalidArguments) {
    const char *argv[] = {"cp", NULL};
    int argc = 1;
    ...
    ASSERT_NE(status, 0) << "cp program should return a non-zero exit status if the number of arguments is less than 3.";

    const char *argv2[] = {"cp", "source.txt", NULL};
    int argc2 = 2;

    result_status = run_cp_command(argc2, const_cast<char**>(argv2));
...
TEST_F(CpTest, SmallerNewFile) {
    ...
    // Run cp command to copy source to destination
    const char *argv[] = {"cp", source, destination, NULL};
    int argc = 3;
    ...
    std::string dest_content = read_file(destination);
    ASSERT_STREQ(new_content, dest_content.c_str()) << "The content of the destination file should be overridden with the new content from the source file (test the case where the destination file already exists while the source file is smaller).";
